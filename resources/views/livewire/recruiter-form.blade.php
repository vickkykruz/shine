<div>
	...
	<script>
		...
		// Desired Job and Preferred Job Form Input
		const desiredJobTitle = document.getElementById('desired-job-title');
		const positionRole = document.getElementById('position-role');
		const responsibilityLevel = document.getElementById('responsibility-level');
		const dPortfolioUrl = document.getElementById('dPortfolio-url');
		const dLinkedinUrl = document.getElementById('dLinkedin-url');
		
		const industrySector = document.getElementById('industry-sector');
		const pPortfolioUrl = document.getElementById('pPortfolio-url');
		const pLinkedinUrl = document.getElementById('pLinkedin-url');
		
		
		
		const initialOptions = Array.from(desiredJob.options).map(option => ({ value: option.value, text: option.text }));
		...
		function fetchJobDecisions() {
            ...
            .then(updatedData => {
				...
				while (desiredJob.options.length > 0) {
					desiredJob.remove(0);
				}
				
                if (updatedData.selectedOption == "yeah") {
					// Add the "No" option if the response is "yeah"
					let option1 = new Option("I have", "year");
					let option2 = new Option("I don't have", "none");
					desiredJob.add(option1);
					desiredJob.add(option2);
					...
					// Set the require attribut for the inputs
					desiredJobTitle.setAttribute('required', 'required');
					positionRole.setAttribute('required', 'required');
					responsibilityLevel.setAttribute('required', 'required');
					dPortfolioUrl.setAttribute('required', 'required');
					dLinkedinUrl.setAttribute('required', 'required');
					
					// Preferred Remove
					industrySector.removeAttribute('required');
					pPortfolioUrl.removeAttribute('required');
					pLinkedinUrl.removeAttribute('required');
				
				} else if (updatedData.selectedOption == "none") {
					// Add the "Yes" option if the response is "none"
					let option1 = new Option("I don't have", "none");
					let option2 = new Option("I have", "yeah");
					desiredJob.add(option1);
					desiredJob.add(option2);
					...
					// Preferred Add require
					industrySector.setAttribute('required', 'required');
					pPortfolioUrl.setAttribute('required', 'required');
					pLinkedinUrl.setAttribute('required', 'required');
					
					// Remove require
					desiredJobTitle.removeAttribute('required');
					positionRole.removeAttribute('required');
					responsibilityLevel.removeAttribute('required');
					dPortfolioUrl.removeAttribute('required');
					dLinkedinUrl.removeAttribute('required');
				}
				loader.style.display = 'none';
            })
            .catch(error => {
		...
		function resetOptions() {
			...
			// Add initial options
			initialOptions.forEach(optionData => {
				let option = new Option(optionData.text, optionData.value);
				desiredJob.add(option);
			});
		}
	</script>
</div>
